...
public interface ConferenceRepository extends JpaRepository<Conference, Integer>{
    ...
    @Query("SELECT MAX(ca.published_year) FROM Conference c INNER JOIN ConferenceArticle ca ON c.conference_ID=ca.conference_ID WHERE c.conference_name=?1")
    Integer findConferenceByLastYear(String conf_name);

    @Query("SELECT MIN(ca.published_year) FROM Conference c INNER JOIN ConferenceArticle ca ON c.conference_ID=ca.conference_ID WHERE c.conference_name=?1")
    ...
    @Query("SELECT COUNT(ath.author_ID) * 1.0 / COUNT(DISTINCT ca.article_ID) FROM Author ath INNER JOIN ConferenceArticle ca ON ath.article_ID=ca.article_ID WHERE ca.conference_name=?1")
    Double findAvgAuthorsByConference(String conf_name);

    @Query("SELECT COUNT(ath.author_ID) * 1.0 / COUNT(DISTINCT ca.published_year) FROM Author ath INNER JOIN ConferenceArticle ca ON ath.article_ID=ca.article_ID WHERE ca.conference_name=?1")
    ...
    @Query("SELECT COUNT(art.article_ID) * 1.0 / COUNT(DISTINCT ca.published_year) FROM Article art INNER JOIN ConferenceArticle ca ON art.article_ID=ca.article_ID WHERE ca.conference_name=?1")
    ...
    @Query("SELECT COUNT(ath.author_ID) * 1.0 / COUNT(DISTINCT ca.article_ID) FROM Author ath INNER JOIN ConferenceArticle ca ON ath.article_ID=ca.article_ID WHERE ca.conference_name=?1 AND ca.published_year>=?2 AND ca.published_year<=?3")
    Double findAvgAuthorsByConferenceRange(String conf_name, int y1, int y2);

    @Query("SELECT COUNT(ath.author_ID) * 1.0 / COUNT(DISTINCT ca.published_year) FROM Author ath INNER JOIN ConferenceArticle ca ON ath.article_ID=ca.article_ID WHERE ca.conference_name=?1 AND ca.published_year>=?2 AND ca.published_year<=?3")
    ...
    @Query("SELECT COUNT(art.article_ID) * 1.0 / COUNT(DISTINCT ca.published_year) FROM Article art INNER JOIN ConferenceArticle ca ON art.article_ID=ca.article_ID WHERE ca.conference_name=?1 AND ca.published_year>=?2 AND ca.published_year<=?3")
    ...
    @Query("SELECT ca.published_year, COUNT(ath.author_ID) / COUNT(DISTINCT ca.article_ID), COUNT(DISTINCT ca.article_ID )FROM ConferenceArticle ca INNER JOIN Author ath ON ca.article_ID=ath.article_ID WHERE ca.conference_name=?1 GROUP BY ca.published_year")
