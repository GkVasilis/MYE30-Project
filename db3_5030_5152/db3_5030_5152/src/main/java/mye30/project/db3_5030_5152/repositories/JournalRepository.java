...
public interface JournalRepository extends JpaRepository<Journal, Integer>{
    ...
    @Query("SELECT  j , MAX(ja.published_year) FROM Journal j INNER JOIN JournalArticle ja ON j.journal_ID=ja.journal_ID WHERE j.journal_name=?1")
    List<Object[]> findJournalByLastYear(String journal_name);

    @Query("SELECT  j , MIN(ja.published_year) FROM Journal j INNER JOIN JournalArticle ja ON j.journal_ID=ja.journal_ID WHERE j.journal_name=?1")
    ...
    @Query("SELECT COUNT(ath.author_ID) / COUNT(ja.article_ID) FROM Author ath INNER JOIN JournalArticle ja ON ath.article_ID=ja.article_ID WHERE ja.journal_name=?1")
    int findAvgAuthorsByJournal(String journal_name);

    @Query("SELECT COUNT(ath.author_ID) / COUNT(ja.published_year) FROM Author ath INNER JOIN JournalArticle ja ON ath.article_ID=ja.article_ID WHERE ja.journal_name=?1")
    ...
    @Query("SELECT COUNT(art.article_ID) / COUNT(ja.published_year) FROM Article art INNER JOIN JournalArticle ja ON art.article_ID=ja.article_ID WHERE ja.journal_name=?1")
    ...
    @Query("SELECT COUNT(ath.author_ID) / COUNT(ja.article_ID) FROM Author ath INNER JOIN JournalArticle ja ON ath.article_ID=ja.article_ID WHERE ja.journal_name=?1 AND ja.published_year>=?2 AND ja.published_year<=?3")
    int findAvgAuthorsByJournalRange(String journal_name, int y1, int y2);

    @Query("SELECT COUNT(ath.author_ID) / COUNT(ja.published_year) FROM Author ath INNER JOIN JournalArticle ja ON ath.article_ID=ja.article_ID WHERE ja.journal_name=?1 AND ja.published_year>=?2 AND ja.published_year<=?3")
    ...
    @Query("SELECT COUNT(art.article_ID) / COUNT(ja.published_year) FROM Article art INNER JOIN JournalArticle ja ON art.article_ID=ja.article_ID WHERE ja.journal_name=?1 AND ja.published_year>=?2 AND ja.published_year<=?3")
    ...
    @Query("SELECT j.publisher, COUNT(j.journal_ID) FROM Journal j WHERE j.publisher=?1")
    int findPublisherPublications(String publisher);

    @Query("SELECT ja.published_year, COUNT(ath.author_ID) / COUNT(ja.article_ID), COUNT (DISTINCT (ja.article_ID) )FROM JournalArticle ja INNER JOIN Author ath ON ja.article_ID=ath.article_ID WHERE ja.journal_name=?1 GROUP BY ja.published_year")
